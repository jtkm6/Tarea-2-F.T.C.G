...
void main(){
	if(interpolation_mode == 2u){
		color = In.color * texture(texture_diffuse, In.coord_texture);
		return;
	}
    vec3 normal = normalize(In.normal);
    vec3 light_direction = normalize(light_position - In.coord_frag);
    ...
    if(diffuse_type == 1u){// Lambert		
		...
		vec3 view_direction = normalize(view_position - In.coord_frag);
	...
	color = (color_light_ambient * color_material_ambient * ambient + color_light_diffuse * color_material_diffuse * diffuse + color_light_specular * color_material_specular * specular) * texture(texture_diffuse, In.coord_texture);
}